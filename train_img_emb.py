...
import h5py
# 自定义mAP计算函数（多标签场景）
def map_score(y_true, y_pred_proba):
    ap_scores = []
...
def moc_map_score(y_true, y_pred_proba):
    ap_scores = []
    for i in range(y_true.shape[1]):
        ap = average_precision_score(y_true[:, i], y_pred_proba[i][:,1])
        ap_scores.append(ap)
    return np.mean(ap_scores)

with h5py.File('dataset/emb_train_data.h5', 'r') as hf:
    X_train = hf['X'][:]
    y_train = hf['y'][:]
...
with h5py.File('dataset/emb_test_data.h5', 'r') as hf:
    X_test = hf['X'][:]
    y_test = hf['y'][:]
...
for model_name, model in model_dict.items():
    ...
    joblib.dump(model, f'emb_model_weight/{model_name}.pkl')
...
for model_name, model in model_dict.items():
    ...
    joblib.dump(model, f'emb_model_weight/{model_name}_text.pkl')
