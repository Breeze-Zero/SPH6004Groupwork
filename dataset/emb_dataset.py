...
if __name__ == "__main__":
    ...
    **{},)
    ...
    # # save .h5
    import h5py
    data_shape = (len(dataset), *dataset[0]['embedding'].shape)  # 假设输入是元组 (X, y)
    label_shape = (len(dataset),*dataset[0]['lab'].shape)
    text_shape = (len(dataset), 512)

    # 创建HDF5文件
    with h5py.File('dataset/emb_train_data.h5', 'w') as hf:
        # 初始化数据集
        ...
        for idx in tqdm(range(len(dataset))):
            data = dataset[idx]
            hf['X'][idx] = data['embedding'].numpy()  # 假设X是Tensor
            hf['y'][idx] = data['lab']
    ...
    label_shape = (len(dataset),*dataset[0]['lab'].shape)
    text_shape = (len(dataset), 512)
    with h5py.File('dataset/emb_val_data.h5', 'w') as hf:
        # 初始化数据集
        hf.create_dataset('X', shape=data_shape, dtype=np.float32)
        hf.create_dataset('y', shape=label_shape, dtype=np.int32)
        hf.create_dataset('X_text', shape=text_shape, dtype=np.float32)
    ...
    label_shape = (len(dataset),*dataset[0]['lab'].shape)
    text_shape = (len(dataset), 512)
    with h5py.File('dataset/emb_test_data.h5', 'w') as hf:
        # 初始化数据集
        hf.create_dataset('X', shape=data_shape, dtype=np.float32)
        hf.create_dataset('y', shape=label_shape, dtype=np.int32)
        hf.create_dataset('X_text', shape=text_shape, dtype=np.float32)
