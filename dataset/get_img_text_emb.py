...
sys.path.append('UniMed-CLIP/src')
...
class MIMIC_Img_text_Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        sample["img"] = load_img(embed_file)
...
if __name__ == "__main__":
    ...
    dataset = MIMIC_Img_text_Dataset(embedpath,csvpath,metacsvpath,splitcsvpath,mode = "train")
    print(dataset[1000])

    # Define main parameters
    model_name = 'ViT-B-16-quickgelu' # available pretrained weights ['ViT-L-14-336-quickgelu', 'ViT-B-16-quickgelu']
    pretrained_weights = "./UniMed-CLIP/src/unimed_clip_vit_b16.pt" # Path to pretrained weights
    ...
    **{},)
    ...
    # # save .h5
    import h5py
    data_shape = (len(dataset), 512)  # 假设输入是元组 (X, y)
    label_shape = (len(dataset),*dataset[0]['lab'].shape)
    text_shape = (len(dataset), 512)

    # 创建HDF5文件
    with h5py.File('dataset/emb_train_data_v2.h5', 'w') as hf:
        # 初始化数据集
        hf.create_dataset('X', shape=data_shape, dtype=np.float32)
        hf.create_dataset('X_text', shape=text_shape, dtype=np.float32)
        hf.create_dataset('X_text_tag', shape=(len(dataset),), dtype=bool)
        hf.create_dataset('y', shape=label_shape, dtype=np.int32)

        # 定义字符串数据类型，指定编码为 UTF-8
        str_dtype = h5py.string_dtype(encoding='utf-8')
        hf.create_dataset('file_name', shape=(len(dataset),), dtype=str_dtype)
        # 遍历Dataset并填充数据
        for idx in tqdm(range(len(dataset))):
            data = dataset[idx]
            hf['file_name'][idx] = data['file_name']
            images = torch.stack([preprocess(data['img'])]).to(device)
            hf['y'][idx] = data['lab']
            with torch.no_grad():
                image_features = model.encode_image(images)
            hf['X'][idx] = image_features[0].cpu()
            if len(data['report'])>0:
                texts = tokenizer(data['report']).to(next(model.parameters()).device, non_blocking=True)
                with torch.no_grad():
                    text_features = model.encode_text(texts)
                    # text_features = text_features / text_features.norm(dim=-1, keepdim=True)
                hf['X_text'][idx] = text_features[0].cpu()
                hf['X_text_tag'][idx] = True
            else:
                hf['X_text_tag'][idx] = False

    dataset = MIMIC_Img_text_Dataset(embedpath,csvpath,metacsvpath,splitcsvpath,mode = "validate")
    data_shape = (len(dataset), 512)  # 假设输入是元组 (X, y)
    label_shape = (len(dataset),*dataset[0]['lab'].shape)
    text_shape = (len(dataset), 512)
    with h5py.File('dataset/emb_val_data_v2.h5', 'w') as hf:
        # 初始化数据集
        hf.create_dataset('X', shape=data_shape, dtype=np.float32)
        hf.create_dataset('X_text', shape=text_shape, dtype=np.float32)
        hf.create_dataset('X_text_tag', shape=(len(dataset),), dtype=bool)
    ...
    dataset = MIMIC_Img_text_Dataset(embedpath,csvpath,metacsvpath,splitcsvpath,mode = "test")
    data_shape = (len(dataset), 512)  # 假设输入是元组 (X, y)
    label_shape = (len(dataset),*dataset[0]['lab'].shape)
    text_shape = (len(dataset), 512)
    with h5py.File('dataset/emb_test_data_v2.h5', 'w') as hf:
        # 初始化数据集
        hf.create_dataset('X', shape=data_shape, dtype=np.float32)
        hf.create_dataset('X_text', shape=text_shape, dtype=np.float32)
        hf.create_dataset('X_text_tag', shape=(len(dataset),), dtype=bool)
